...
namespace VisAssets.SciVis.Structured.Isosurface
{
	...
	// =========================================================================
#if UNITY_EDITOR
	[CustomEditor(typeof(IsosurfaceMod))]
	public class IsosurfaceModEditor : Editor
	{
		...
		{
			var isosurface = target as IsosurfaceMod;
			...
			{
				Undo.RecordObject(target, "IsosurfaceMod");
				if (isosurface != null)
				{
	...
	// Main Class
	// =========================================================================
	public class IsosurfaceMod : MapperModuleTemplate
	{
...
//var indexBuffer = new GraphicsBuffer(type, packedTables.Length, sizeof(int));

UnityEngine.Debug.Log("packedTables.Length = " + packedTables.Length);
var _packedTables = new int[packedTables.Length];

			tablesBuffer = new ComputeBuffer(packedTables.Length, sizeof(int)); // PVRSRVFreeDeviceMemMIW error #2

			tablesBuffer.SetData(packedTables);
			tablesBuffer.GetData(_packedTables);

			bool ok = true;
			for (var i = 0; i < packedTables.Length; i++)
			{
				if (packedTables[i] != _packedTables[i])
				{
					UnityEngine.Debug.Log("error in " + i);
					ok = false;
				}
			}
			if (ok) UnityEngine.Debug.Log("compute buffer check ok");
			...
			vertexBufferSingle       = singleMesh.GetVertexBuffer(0); // PVRSRVFreeDeviceMemMIW error #2
			counterBufferSingle      = new ComputeBuffer(1, sizeof(uint), ComputeBufferType.Counter); // PVRSRVFreeDeviceMemMIW error #3, 4
			counterCheckBufferSingle = new ComputeBuffer(1, sizeof(uint), ComputeBufferType.IndirectArguments); // PVRSRVFreeDeviceMemMIW error #5
...
UnityEngine.Debug.Log("SetParameter");
		...
		{
			if (!pdf.dataLoaded) return;
UnityEngine.Debug.Log("ReSetParameter");
			StopAllCoroutines();
...
//				activation.SetParameterChanged(ModuleState.PARAMETER_CHANGED);
		...
		public void Calc()
		{
UnityEngine.Debug.Log("Calc_0");
			if (values == null || coords == null || cvBufferSingle == null) return;
UnityEngine.Debug.Log("Calc_1");
...
UnityEngine.Debug.Log("Calc_2");
			StartCoroutine(SinglePassCalcRoutine());
UnityEngine.Debug.Log("Calc_3");
UnityEngine.Debug.Log("triCount = " + triCount);
		}
		...
		private void GenCoordPrepSinglePass()
		{
			...
			{
				if (cvBufferSingle != null) cvBufferSingle.Dispose();
				cvBufferSingle = new ComputeBuffer(volumeSize, sizeof(float) * 4); // PVRSRVFreeDeviceMemMIW error
		...
		private IEnumerator SinglePassCalcRoutine()
		{
			...
			uint sx, sy, sz;
			shader.GetKernelThreadGroupSizes(kernel, out sx, out sy, out sz); // PVRSRVFreeDeviceMemMIW error
			int _x = (dims[0] + (int)sx - 1) / (int)sx;
			int _y = (dims[1] + (int)sy - 1) / (int)sy;
			int _z = (dims[2] + (int)sz - 1) / (int)sz;
UnityEngine.Debug.Log("sx = " + sx);
UnityEngine.Debug.Log("sy = " + sy);
UnityEngine.Debug.Log("sz = " + sz);
			shader.Dispatch(kernel, _x, _y, _z); // PVRSRVFreeDeviceMemMIW error
